...
namespace EasyPDV.UI
{
    public partial class FrmSale : Form
    {
            ...
            if (res == DialogResult.OK)
            {
                foreach (DataGridViewRow row in salesGridView1.Rows)
                {
                    if (row.Selected)
                    {
                        ...
                        {
                            individualSaleDAO.DeleteIndividualSale(itemsInList[i], int.Parse(row.Cells[0].Value.ToString()));
                        }
                    }
                }
                ShowSales();
        ...
        {
            string fileName = "\\Relatório Vendas Realizadas - Caixa nº " + cashierOpenDAO.ReturnCashierNumber() + " - " + DateTime.Now.ToString("dd-MM-yyyy") + ".xlsx";
            string path = "";
